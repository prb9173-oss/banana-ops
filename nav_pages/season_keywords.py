...
# 짧은 TTL 캐시: 매장/묶음 드롭다운을 바꾸거나 확인창을 열고 닫을 때마다
# 매번 네이버 API를 새로 호출하면 그때마다 로딩이 느껴진다. 실제로 On/Off를
# 적용한 직후에는 아래에서 .clear()로 캐시를 지워 최신 상태를 다시 받아온다.
@st.cache_data(ttl=8)
...
# [매장 + 묶음 선택]
# ==========================================
# 이 구역 전체를 st.fragment로 감싸서, 드롭다운 변경/On·Off 버튼/확인창
# 클릭이 페이지 전체(묶음 추가 폼, 저장된 묶음 목록 등)를 다시 그리지 않고
# 이 카드 안에서만 다시 그려지게 한다 — 클릭할 때마다 페이지가 통째로
# 새로고침되는 것처럼 느껴지던 지연의 주된 원인이었다.
@st.fragment
def _render_store_mgmt(stores, bundles):
    store_options = {s["store_name"]: s for s in stores}

    col_store, col_bundle = st.columns(2)
    with col_store:
        selected_store = st.selectbox(
            "매장 선택", options=list(store_options.keys()), key="sk_selected_store",
        )
    with col_bundle:
    ...
    def _confirm_toggle_dialog(pending):
        ...
        st.markdown(
            f"정말 **{selected_store}** 매장에 '**{selected_bundle['name']}**' 묶음을 "
            f"**{action_label}** 하시겠습니까?"
        )
        st.caption("실제 네이버 광고 계정(파워링크)에 바로 반영됩니다.")
...
with st.container(border=True, key="section_store_mgmt"):
    ...
    if not st.session_state.get("is_admin"):
        ...
        if not stores:
            ...
            _render_store_mgmt(stores, bundles)
...
with st.container(border=True, key="section_bundle_list"):
    ...
    @st.dialog("묶음 삭제 확인")
    def _confirm_delete_bundle_dialog(bundle):
        st.markdown(f"정말 '**{bundle['name']}**' 묶음을 삭제하시겠습니까?")
        st.caption(f"키워드 {len(bundle['keywords'])}개가 함께 삭제되며, 되돌릴 수 없습니다.")
        col_yes, col_no = st.columns(2)
        with col_yes:
            if st.button("삭제", key=f"confirm_delete_yes_{bundle['id']}", width="stretch"):
                get_supabase_client().table("season_keyword_bundles").delete().eq("id", bundle["id"]).execute()
                fetch_bundles.clear()
                st.session_state[f"confirm_delete_{bundle['id']}"] = False
                st.rerun()
        with col_no:
            if st.button("취소", key=f"cancel_delete_{bundle['id']}", width="stretch"):
                st.session_state[f"confirm_delete_{bundle['id']}"] = False
                st.rerun()
    ...
    if not bundles:
        ...
        for bundle in filtered_bundles:
            ...
            with st.container(border=True, key=f"bundle_card_{bundle['id']}_{card_state}"):
                    ...
                    with st.container(key=f"actions_{bundle['id']}"):
                        if st.session_state.get("is_admin"):
                            ...
                            if st.button(":material/delete:", key=f"delete_{bundle['id']}", help="삭제"):
                                st.session_state[f"confirm_delete_{bundle['id']}"] = True
                ...
                if st.session_state.get(f"confirm_delete_{bundle['id']}"):
                    _confirm_delete_bundle_dialog(bundle)

                if st.session_state.get("is_admin") and st.session_state.get(f"editing_{bundle['id']}", False):
                    with st.container(key=f"editpanel_{bundle['id']}"):
                        st.markdown('<div class="edit-panel-label">✏️ 키워드 수정 (한 줄에 하나씩 입력)</div>', unsafe_allow_html=True)
                        ...
                        with st.container(key=f"editform_actions_{bundle['id']}"):
                            if st.button("저장", key=f"save_{bundle['id']}"):
                                new_kws = [kw.strip() for kw in edited_kw_raw.splitlines() if kw.strip()]
                                if not new_kws:
